...
class HJBSolver:
    ...
    def solve(self):
        ...
        for t_idx in range(self.n_T - 2, -1, -1):
            ...
            threshold = 0.001 * self.K  # More strict: 0.1% of strike
...
def main():
    ...
    for symbol in SYMBOLS:
        ...
        for reg_idx in range(n_regimes):
            label = regime_labels[reg_idx]

            print(f"\n{'=' * 70}")
            print(f"REGIME: {label}")
            print('=' * 70)

            rp = loader.get_regime_params(reg_idx)
            jp = loader.get_jump_params(symbol, reg_idx)

            # Extract lambda from jump params, not regime params!
            ...
            try:
                solver = HJBSolver(
                    ...
                    sigma=rp['sigma'],
                    lambda_j=lambda_j,  # Use jump-detected lambda
                    mu_J=jp['mu_J'],
                    sigma_J=jp['sigma_J']
                )
                ...
                results['solutions'][symbol][reg_idx] = {
                    'label': label,
                    'hjb': hjb_solution,
                    'contract': contract,
                    'regime_params': rp,
    ...
    for sym, sol in results['solutions'].items():
        print(f"\n{sym}:")
        for reg_idx, data in sol.items():
            hjb = data['hjb']
            print(f"  {data['label']}: V=${hjb['american']:.4f}, "
                  f"Boundary=${hjb['boundary_t0']:.2f} "
