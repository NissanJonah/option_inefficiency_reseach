...
class FokkerPlanckSolver:
    ...
    def __init__(self, S0, T, regime_params, jump_params, r=0.04, q=0.0):
        self.S0 = S0
        self.T = T
        self.r = r
        self.q = q
    ...
    def _setup_grid(self):
        ...
        self.S_grid = np.linspace(S_min, S_max, CONFIG['n_S_points'])
    ...
    def solve(self):
        ...
        def rhs(p, t):
            ...
            drift_deriv = np.gradient(drift, self.dS)

            # Diffusion term: ½∂²/∂S²[σ²S²·p]
            diff = 0.5 * self.sigma ** 2 * S ** 2 * p
            diff_deriv2 = np.gradient(np.gradient(diff, self.dS), self.dS)
...
class ImpliedDistributionExtractor:
    ...
    def extract(self, symbol, date):
        ...
        d2C_dK2 = np.gradient(np.gradient(C_dense, dK), dK)
